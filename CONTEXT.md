...
**Failure Item**:
`Continue On Fail` açıkken bir İstek Hatası'nı kararlı hata kodu ve redakte edilmiş mesajla temsil eden, binary içermeyen ve kaynak input'a bağlı output item. Her iki continue modunda da tek main output'a düşer; `onError: continueErrorOutput` altında n8n'in eklediği error output engine'e ait olduğundan node oraya yazamaz ve execution hint'i ile uyarır.

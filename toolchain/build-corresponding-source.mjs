...
async function vendorCargo(
  ...
  if (cacheHit) {
    ...
    await run("docker", [
      ...
      "/bin/sh",
      "-ec",
      'apk add --no-cache cargo git ca-certificates >/dev/null; cargo vendor --quiet --locked /out/vendor --manifest-path "$1/Cargo.toml" >/out/vendor-config.toml; chown -R "$HOST_UID:$HOST_GID" /out',
      "source-vendor",

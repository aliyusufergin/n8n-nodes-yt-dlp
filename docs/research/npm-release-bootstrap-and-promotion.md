...
6. **Kanıtlanmış platform gerçeği:** Publishing without an explicit tag sets `latest`; `npm install <name>` resolves `latest`. Publishing or staging with `--tag next` avoids moving an existing `latest`, and `next` has no special registry semantics. The registry requires every package to have a `latest` tag, so a package's first published version may receive both `next` and `latest`; this is not an explicit promotion. Sources: [`npm dist-tag`](https://docs.npmjs.com/cli/v11/commands/npm-dist-tag/), [`npm registry package metadata`](https://github.com/npm/registry/blob/master/docs/responses/package-metadata.md), accessed 2026-07-29.

...
Re-verified against the 2.34.5 source on 2026-08-13 after the head advance.
`community-packages.service.ts` is byte-identical to 2.30.7, so the install-gate
findings carry over without change. The n8n-docs snapshot was not re-cut; the
mutable-documentation findings still date from 2026-07-17.
